				       ...
				       if(empty($_POST['score0']) && empty($_POST['user_id']) )
					    {
							...
							$message_erro = $message_erro . "The field can not be empty";
							 ...
							 $sql = "DELETE FROM courses_registered WHERE  $admission_no = $user_exam_no ";
							 $result = mysql_query($sql);
							 if($result)
							 {
							   $message_erro = "Congratulation!!!"."<br>";
							   $message_erro = $message_erro . "Student marks have been Deleted";
							  }else{
									$message_erro = "Error in Deleting student scores check the scores and save again";
							  }
					...
					if(empty($_POST['score0']) && empty($_POST['score1']) && empty($_POST['score2']) && empty($_POST['user_id']))
					{
						...
						$message_erro = $message_erro. "The field can not be empty";
							 ...
							 if($result)
							 {
							   ...
							   echo "Student marks have been Deleted";
							  }else{
									$message_erro = "Error in Deleting student scores check the scores and save again";
								  }
						}
							 ...
							 $result = mysql_query($sql);
								 ...
								 if($result)
								 {
								   ...
								   $message_erro =$message_erro . "Student marks have been Deleted";
										...
										echo "Error in Deleting student scores check the scores and save again";
									  }
							 }
				break;
				     ...
				     if(empty($_POST['score0']) && empty($_POST['score1']) && empty($_POST['score2']) && empty($_POST['score3']) && empty($_POST['score4']) && empty($_POST['score5']) && empty($_POST['score6']) && empty($_POST['user_id']))
						{
							...
							$message_erro =$message_erro. "The field can not be empty";
								 ...
								 {
								   $message_erro = "Congratulation!!!"."<br>";
								   $message_erro = "Student marks have been Deleted";
								  }else{
